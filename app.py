...
from components.v55_macro_additions import render_v55_macro_additions  # v55 additive macro expansion
...
from config.indicator_additions_v55 import V55_DEFAULT_IDS, V55_DERIVED_IDS  # v55 additive macro expansion
...
from utils.v55_derived import build_v55_derived_indicators  # v55 additive macro expansion
...
def cached_load(user: str, _password: str, start: str, end: str, indicator_ids: tuple[str, ...], cache_token: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    data = build_v55_derived_indicators(data)
...
def top_controls(creds: Credentials, catalog_df: pd.DataFrame) -> tuple[date, date, list[str], str, dict]:
    ...
    # v55 additive macro expansion: añadir nuevos indicadores por defecto sin remover los existentes.
    selected_ids = list(dict.fromkeys(selected_ids + V55_DEFAULT_IDS))
...
def filter_data(df: pd.DataFrame, ids: list[str], frequency: str) -> pd.DataFrame:
    ...
    keep_ids = ids + ["tasa_real_ex_post", "pendiente_nominal", "pendiente_uf"] + V55_DERIVED_IDS
...
def main() -> None:
    ...
    for did in ["tasa_real_ex_post", "pendiente_nominal", "pendiente_uf"] + V55_DERIVED_IDS:
    ...
    tabs = st.tabs([
        ...
        "8. Derivados",
        "9. Macro ampliado v55",
        "10. Señales",
        "11. Calidad / descargas",
        "12. Metodología",
    ])
    ...
    with tabs[8]:
        export_sheets.update(render_v55_macro_additions(data, opts))
    with tabs[9]:
...
# v55 additive macro expansion applied
